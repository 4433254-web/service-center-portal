...
import { ordersApi, clientsApi, devicesApi, usersApi, locationsApi } from '@/lib/api';
...
export default function NewOrderPage() {
  ...
  const [orderForm, setOrderForm] = useState({
    ...
    masterUserId: '', locationId: '',
  });

  useEffect(() => {
    usersApi.list().then(u => setMasters(u.filter((x: any) =>
      x.roles?.includes('master') || x.roles?.includes('receiver') || x.roles?.includes('admin')
    ))).catch(() => {});
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const order = await ordersApi.create({
        ...
        conditionAtAcceptance: orderForm.conditionAtAcceptance,
        includedItems: orderForm.includedItems || undefined,
        ...
        receiverComment: orderForm.receiverComment || undefined,
        masterUserId: orderForm.masterUserId || undefined,
        ...
        {step === 'order' && (
          <form onSubmit={handleSubmit} className="card p-6 space-y-4">
            <div><label className="label">Заявленная неисправность *</label>
              <textarea className="input" rows={3} required value={orderForm.issueDescription} onChange={e => setOrderForm(f=>({...f,issueDescription:e.target.value}))} placeholder="Не включается, разбит экран, нужна чистка динамика…" /></div>
            <div><label className="label">Состояние при приёме *</label>
              <textarea className="input" rows={2} required value={orderForm.conditionAtAcceptance} onChange={e => setOrderForm(f=>({...f,conditionAtAcceptance:e.target.value}))} placeholder="Царапины на корпусе, трещина на экране…" /></div>
            <div><label className="label">Комплектация</label>
              <input className="input" value={orderForm.includedItems} onChange={e => setOrderForm(f=>({...f,includedItems:e.target.value}))} placeholder="Кабель, чехол, СЗУ…" /></div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <select className="input" value={orderForm.masterUserId} onChange={e => setOrderForm(f=>({...f,masterUserId:e.target.value}))}>
                  <option value="">— не назначен —</option>
                  {masters.map(m => <option key={m.id} value={m.id}>{m.login} ({(m.roles||[]).join(', ')})</option>)}
                </select>
              </div>
              <div>
                <label className="label">Точка приёма</label>
                <select className="input" value={orderForm.locationId} onChange={e => setOrderForm(f=>({...f,locationId:e.target.value}))}>
            ...
            <div><label className="label">Комментарий приёмщика</label>
              <textarea className="input" rows={2} value={orderForm.receiverComment} onChange={e => setOrderForm(f=>({...f,receiverComment:e.target.value}))} placeholder="Клиент просит позвонить перед ремонтом…" /></div>

            <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 text-xs text-blue-700">
              💡 После создания заказа можно сразу изменить статус на «В работе» или «Готов» — без диагностики
            </div>
